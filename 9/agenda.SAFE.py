"""
   agenda.py: CIS 210 assignment 7, Winter 2015
author: Michael Harris, Harris@uoregon.edu
...
class Agenda:
    ...
    def normalize(self):
        """Takes two appointments and returns a new appt from the start and
        end constraints"""
        ordering = lambda appt: appt.begin
        self.appts.sort(key=ordering)
        prev_appt = self.appts[0]
        normalized_agenda = []
        for appt in self:
            print (appt)
            if prev_appt.overlaps(appt):
                print ("overlap")
                print ("prev_appt beg ==", prev_appt.begin)
                print ("prev_appt end ==", prev_appt.end)
                start_times = [prev_appt.begin, appt.begin]
                #print ("start == ",start_times)
                end_times = [prev_appt.end, appt.end]
                #print ("end ==",end_times)
                prev_appt = Appt(prev_appt.end.date(), min(start_times).time(), max(end_times).time(), appt.desc)
                print ("prev_appt 1 ==", prev_appt)
            else:
                normalized_agenda.append(prev_appt)
                prev_appt = appt
                print ("no overlap")
                print ("prev_appt 2 ==", prev_appt)
                print ("normalized_agenda ==", normalized_agenda)
        print ("final normalized_agenda == ", normalized_agenda)
        self.appts = normalized_agenda
        print (self.appts)
               
        
    def complement(self, freeblock):
        """Creates a new agenda using the end times of the previous
        appt as the start time for new appts and the start times
        of the previous appt as the end times for new appts. This creates
        a "compliment", an agenda that uses all open times of the
        original agenda as unavailable times"""
        agenda_copy = Agenda()
        agenda_copy.appts = self.appts
        #agenda_copy = copy.deepcopy(self.appts) original copy method, found in google
        print ("agenda_copy ==", agenda_copy)
        normalized = agenda_copy.normalize() #NORMALIZE IS NOT RUNNING!
        print ("normalized ==", self.appts)
        prev_appt = agenda_copy.appts[0]
        compliment = []
        for appt in agenda_copy.appts:
            compliment.append(Appt(appt.begin.date(), prev_appt.end.time(), appt.begin.time(), appt.desc))
            prev_appt = appt
        self.appts = compliment

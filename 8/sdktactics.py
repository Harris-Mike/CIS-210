...
sdktactics.py: CIS 211 assignment 8, Winter 2015
author: Michael Harris, Harris@uoregon.edu
Consulted with: Anna Gladstone: assisted me withstructuring my naked_single function
                Katie Lillard: consulted with on col in function prepare board
...
def naked_single(group):
    ...
    # Removes all symbols in set seen from each blank tiles
    for tile in group:
        if tile.symbol == sdkboard.OPEN:
...
def hidden_single(group):
    ...
    # Checks each number from need_placed to see if only one of
    # the nine tiles in group has number in tile.possible
